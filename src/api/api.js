...
const realApi = {
  ...
  async createUser(username, role_id, group_id) {
    // Generate a random password
    const randomPassword = Math.random().toString(36).slice(-8);
    const res = await fetch('http://localhost:4000/api/users', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username, password: randomPassword, role_id, group_id, must_change_password: true }),
    });
    if (!res.ok) {
    ...
    return { success: true, newUser, password: randomPassword };
  },
  ...
  async resetUserPassword(userId) {
    ...
    const res = await fetch(`http://localhost:4000/api/users/${userId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ password: newPassword, must_change_password: true }),
    });
    if (!res.ok) {
